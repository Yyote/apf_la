...
int velocity_parameter_pass_counter = 0;
int odom_parameter_pass_counter = 0;
...
void vel_cb(const geometry_msgs::Twist::ConstPtr& velocity)
{
    ...
    {
        velocity_parameter_pass_counter++;
    }
}
...
void odom_cb (const nav_msgs::Odometry::ConstPtr& odom)
{
    ...
    // int j = 0;
    while (bufferized_odometry.at(0).header.stamp.toSec() < ros::Time::now().toSec() - buffer_life_length)
    {
        bufferized_odometry.erase(bufferized_odometry.begin());
        odom_parameter_pass_counter++;
    }

    if (odom_parameter_pass_counter >= 150)
    {
        apf_la::GetTrajectory srv;
        ...
        if (client.call(srv))
        {
            ...
            trajectory_pub.publish(trajectory);
            ROS_INFO_STREAM("Published trajectory with" << srv.response.path.size() << "waypoints\n");
            velocity_parameter_pass_counter = 0;
        }
    ...
    if(velocity_parameter_pass_counter >= 25)
    {
        apf_la::GetTrajectory srv;
        srv.request.goal_position.pose.position.x = bufferized_goal.pose.position.x;
        srv.request.goal_position.pose.position.y = bufferized_goal.pose.position.y;
